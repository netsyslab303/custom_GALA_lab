...
def save_as_image(input, output, batch, noised_frame, noised_joint):
    ...
    for i in range(len(inp)):
        ...
        np.save(image_dir+"/noised_frame_{}_{}_{}".format(noised_frame[i][0],noised_frame[i][1],noised_frame[i][2]), noised_frame[i])
        for j in (noised_frame[i]):
            ...
            plt.savefig(os.path.join(image_dir, 'out_{}.pdf'.format(j)))
            plt.close()
            #plt.show()
